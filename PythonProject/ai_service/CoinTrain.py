...
def extract_data(raw_data,new_data=True,coin_name='BTC'):
    ...
    if new_data:
        ...
        with open(os.path.join(COIN_PATH, f"{coin_name.lower()}_robust_scaler.pkl"),"wb") as fp:
            pickle.dump(robust_scaler,fp) 
    else :
        with open(os.path.join(COIN_PATH, f"{coin_name.lower()}_robust_scaler.pkl"),"rb") as fp:
...
# shuffle = False 를 넣을 수 있다..넣는게 좋지 않나..?
...
def result_graph(fit_history,coin_name):
    ...
    if not os.path.exists(f"{COIN_PATH}static"):
        os.makedirs("{COIN_PATH}static/chart")
...
if __name__=="__main__":
    ...
    for coin_name in coin_names:
        ...
        # 모델 저장
        print(model)
        with open(r"{}coin_config/{}_err_rate".format(coin_name.lower()),"wb") as fp:
